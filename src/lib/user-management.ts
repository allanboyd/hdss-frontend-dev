...
export const userProfileService = {
  ...
  async update(
    id: number,
    updates: Partial<CreateUserProfileForm>
  ): Promise<UserProfile> {
    try {
      const { data, error } = await supabaseAdmin
        .from('user_profile')
...
export const siteUserService = {
  ...
  async update(
    id: number,
    updates: Partial<CreateSiteUserForm>
  ): Promise<SiteUser> {
    try {
      const { data, error } = await supabaseAdmin
        .from('site_user')
...
export const userAccountRequestService = {
  ...
  async create(
    request: CreateUserAccountRequestForm
  ): Promise<UserAccountRequest> {
    try {
      const { data, error } = await supabaseAdmin
        .from('user_account_request')
  ...
  async update(
    id: number,
    updates: UpdateUserAccountRequestForm
  ): Promise<UserAccountRequest> {
    try {
      const { data, error } = await supabaseAdmin
        .from('user_account_request')
      ...
      if (error) {
        ...
        if (
          ...
          error.message?.includes(
            'relation "user_management_counts" does not exist'
          ) ||
          ...
          console.warn(
            'user_management_counts view not found, returning default counts'
          );
          return {
            roles_count: 0,
            users_count: 0,
