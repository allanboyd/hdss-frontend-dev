...
export function InteractiveMap({ className = '' }: InteractiveMapProps) {
  ...
  useEffect(() => {
    ...
    const initializeMap = async () => {
      ...
      // Import Leaflet CSS dynamically (only if not already loaded)
      ...
      countries.forEach(country => {
        ...
        const bounds = L.default.latLngBounds(
          country.coordinates as [number, number][]
        );
      ...
      // Add legend
      const legend = L.default.Control.extend({
        onAdd: function () {
          const div = L.default.DomUtil.create(
            'div',
            'legend bg-white bg-opacity-90 rounded p-2 text-xs shadow-lg border border-orange-200'
          );
          div.innerHTML = `
  ...
  if (!isClient) {
    return (
      <div
        className={`w-full h-full rounded-b-lg bg-gray-100 flex items-center justify-center ${className}`}
        style={{
          minHeight: '320px',
          position: 'relative',
        }}
