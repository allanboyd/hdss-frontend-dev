...
import { Structure, CreateStructureForm, Village } from "@/types/site-management"
...
export function StructuresTab({ searchQuery }: StructuresTabProps) {
  ...
  const handleAdd = async () => {
    if (!formData.structure_code.trim() || !formData.village_id) {
      toast.error("Please fill in all required fields (Structure Code and Village)")
      return
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (errorMessage.includes('foreign key') || errorMessage.includes('constraint')) {
        toast.error("Cannot delete structure: It is being used by other records (dwelling units, etc.)")
      } else if (errorMessage.includes('permission') || errorMessage.includes('unauthorized')) {
        toast.error("Permission denied: You don't have access to delete this structure")
      } else {
  ...
  const filteredStructures = structures.filter(structure =>
    structure.structure_code.toLowerCase().includes(searchQuery.toLowerCase()) ||
    structure.structure_name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
    structure.village?.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
    structure.village?.site?.name.toLowerCase().includes(searchQuery.toLowerCase())
  )
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Manage buildings and structures within villages</p>
        </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) => setFormData({ ...formData, village_id: parseInt(value) })}
                >
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    {villages.map((village) => (
                      <SelectItem key={village.village_id} value={village.village_id.toString()}>
                        {village.name} - {village.site?.name} ({village.site?.country?.name})
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData({ ...formData, structure_code: e.target.value })}
                ...
                <TableRow key={structure.structure_id}>
                  <TableCell>
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        <span className="font-medium">{structure.structure_code}</span>
                        {structure.structure_name && (
                          <p className="text-sm text-gray-500">{structure.structure_name}</p>
                        )}
                      </div>
                    </div>
                  </TableCell>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-4">
            ...
            <div>
              <Label htmlFor="edit-address_description">Address Description</Label>
              <Textarea
